...
export async function ensureSchema() {
  ...
  // Suppression "douce" des sites/postes deja utilises par des vacations ou
  // un planning : la ligne est gardee (l'historique et les exports y font
  // reference) mais masquee partout dans l'application.
  await sql`ALTER TABLE sites ADD COLUMN IF NOT EXISTS supprime BOOLEAN NOT NULL DEFAULT false;`;
  await sql`ALTER TABLE postes ADD COLUMN IF NOT EXISTS supprime BOOLEAN NOT NULL DEFAULT false;`;
  ...
  // Planning site (besoins du marche) : creneaux de travail prevus sur un
  // site, d'abord "a pourvoir", puis attribues a un agent. L'attribution
  // n'est pas stockee ici : c'est la ligne planning_entries qui porte
  // site_slot_id qui fait foi. Ainsi, attribuer un agent = lui creer
  // automatiquement ce creneau dans son planning ; et si le creneau est
  // efface de son planning, il redevient "a pourvoir" cote site.
  await sql`
    CREATE TABLE IF NOT EXISTS site_slots (
      id SERIAL PRIMARY KEY,
      site_id INTEGER NOT NULL REFERENCES sites(id) ON DELETE CASCADE,
      poste_id INTEGER REFERENCES postes(id) ON DELETE SET NULL,
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS idx_site_slots_site_date ON site_slots(site_id, slot_date);`;
  // Lot de creation (tous les creneaux crees en une fois) et jour d'origine
  // de la vacation : permettent de supprimer "toute la vacation" d'un coup.
  await sql`ALTER TABLE site_slots ADD COLUMN IF NOT EXISTS lot_id TEXT;`;
  await sql`ALTER TABLE site_slots ADD COLUMN IF NOT EXISTS lot_origine DATE;`;
  await sql`CREATE INDEX IF NOT EXISTS idx_site_slots_lot ON site_slots(lot_id);`;
  await sql`ALTER TABLE planning_entries ADD COLUMN IF NOT EXISTS site_slot_id INTEGER REFERENCES site_slots(id) ON DELETE CASCADE;`;
  // Vacation creee a partir d'un creneau du planning ("marquer comme
  // effectue") : evite de compter deux fois le meme creneau.
  await sql`ALTER TABLE shifts ADD COLUMN IF NOT EXISTS planning_entry_id INTEGER REFERENCES planning_entries(id) ON DELETE SET NULL;`;
  await sql`CREATE UNIQUE INDEX IF NOT EXISTS idx_shifts_planning_entry ON shifts(planning_entry_id) WHERE planning_entry_id IS NOT NULL;`;
  await sql`CREATE UNIQUE INDEX IF NOT EXISTS idx_planning_site_slot ON planning_entries(site_slot_id) WHERE site_slot_id IS NOT NULL;`;

  // Base salaries (vivier de profils) : fiches independantes des comptes de
  // pointage (pas de code de connexion). La photo et le CV sont stockes
  // dans un store Vercel Blob PRIVE : on ne garde ici que leur chemin
  // (pathname), et les fichiers ne sont servis qu'au responsable connecte
  // via /api/admin/base-salaries/fichier.
  await sql`
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS idx_staff_profiles_nom ON staff_profiles(nom);`;
  // Numero de securite sociale (ajoute apres la creation de la table).
  await sql`ALTER TABLE staff_profiles ADD COLUMN IF NOT EXISTS num_secu TEXT;`;
  // Salarie effectif (compte de pointage) cree a partir de cette fiche.
  await sql`ALTER TABLE staff_profiles ADD COLUMN IF NOT EXISTS employee_id INTEGER REFERENCES employees(id) ON DELETE SET NULL;`;
  // Coordonnees bancaires et passeport (fichier dans Vercel Blob prive).
  await sql`ALTER TABLE staff_profiles ADD COLUMN IF NOT EXISTS iban TEXT;`;
  await sql`ALTER TABLE staff_profiles ADD COLUMN IF NOT EXISTS bic TEXT;`;
  await sql`ALTER TABLE staff_profiles ADD COLUMN IF NOT EXISTS passeport_pathname TEXT;`;
  await sql`ALTER TABLE staff_profiles ADD COLUMN IF NOT EXISTS passeport_nom_fichier TEXT;`;
